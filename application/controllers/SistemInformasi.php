...
class SistemInformasi extends CI_Controller
{
    public function __construct()
    {
        ...
        $this->load->model('Model_sisteminformasi', 'sisteminformasi');
    ...
    public function index()
    {
        ...
        $data['sisteminformasis'] = $this->sisteminformasi->getData();

        $this->load->view('layouts/header', $data);
        $this->load->view('layouts/sidebar');
        $this->load->view('layouts/topbar');
        $this->load->view('dashboard/sisteminformasi/index', $data);
    ...
    {
        $data['title'] = 'sistem informasi | Tambah sistem informasi';
     
        $this->load->view('layouts/header', $data);
        $this->load->view('layouts/sidebar');
        $this->load->view('layouts/topbar');
        $this->load->view('dashboard/sisteminformasi/create', $data);
        $this->load->view('layouts/footer');
    }
    ...
    public function store()
    {
        ...
        if ($this->form_validation->run() === false) {
            ...
            if (!$this->upload->do_upload('foto')) {
                ...
                redirect('sisteminformasi/create');
            } else {
        ...
        'tendik_kontrak' => htmlspecialchars($this->input->post('tendik_kontrak')),
            ...
            $this->sisteminformasi->storeData($data);

                if ($this->db->affected_rows() > 0) {
                    $this->session->set_flashdata('success', 'Berhasil menambah data!');
                    redirect('sisteminformasi');
                } else {
    ...
    {
        $data['title'] = 'sistem informasi | Update sistem informasi';
        $data['data'] = $this->sisteminformasi->editData($id);

        $this->load->view('layouts/header', $data);
        $this->load->view('layouts/sidebar');
        $this->load->view('layouts/topbar');
        $this->load->view('dashboard/sisteminformasi/edit', $data);
        $this->load->view('layouts/footer');
    }
    ...
    public function update($id)
    {
        ...
        if ($this->form_validation->run() === false) {
            ...
            if (!empty($_FILES['foto']['name'])) {
                ...
                if (!$this->upload->do_upload('foto')) {
                    $this->session->set_flashdata('error', $this->upload->display_errors());
                    redirect('sisteminformasi/edit/' . $id);
                } else {
                    $fileData = $this->upload->data();
                    $foto = $fileData['file_name'];
                }
        ...
        'tendik_kontrak' => htmlspecialchars($this->input->post('tendik_kontrak')),
            ...
            $this->sisteminformasi->updateData($id, $data);
    ...
    {
        $this->sisteminformasi->destroyData($id);

        if ($this->db->affected_rows() > 0) {
            $this->session->set_flashdata('success', 'Berhasil menghapus sisteminformasi!');
            redirect('sisteminformasi');
        } else {
            $this->session->set_flashdata('error', 'Gagal menghapus sisteminformasi!');
            redirect('sisteminformasi');
        }
    }
    ...
    {
        $data['title'] = 'sistem informasi | Detail sistem informasi';
        $data['data'] = $this->sisteminformasi->showData($id);

        $this->load->view('layouts/header', $data);
        $this->load->view('layouts/sidebar');
        $this->load->view('layouts/topbar');
        $this->load->view('dashboard/sisteminformasi/show', $data);
        $this->load->view('layouts/footer');
    }
