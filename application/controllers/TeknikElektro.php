...
class TeknikElektro extends CI_Controller
{
    public function __construct()
    {
        parent::__construct();
        isLoggedIn($this->session->userdata('id'));
        $this->load->model('Model_teknikelektro', 'teknikelektro');
        $this->load->library('form_validation');
    }

    public function index()
    {
        $data['title'] = 'Teknik elektro';
        $data['teknikelektros'] = $this->teknikelektro->getData();

        $this->load->view('layouts/header', $data);
        $this->load->view('layouts/sidebar');
        $this->load->view('layouts/topbar');
        $this->load->view('dashboard/teknikelektro/index', $data);
    ...
    {
        $data['title'] = 'Teknik elektro | Tambah Teknik elektro';
     
        $this->load->view('layouts/header', $data);
        $this->load->view('layouts/sidebar');
        $this->load->view('layouts/topbar');
        $this->load->view('dashboard/teknikelektro/create', $data);
        $this->load->view('layouts/footer');
    }
    ...
    public function store()
    {
        ...
        'tendik_kontrak' => htmlspecialchars($this->input->post('tendik_kontrak')),
            ...
            $this->teknikelektro->storeData($data);

                if ($this->db->affected_rows() > 0) {
                    $this->session->set_flashdata('success', 'Berhasil menambah data!');
                    redirect('teknikelektro');
                } else {
    ...
    {
        $data['title'] = 'Teknik elektro | Update Teknik elektro';
        $data['data'] = $this->teknikelektro->editData($id);

        $this->load->view('layouts/header', $data);
        $this->load->view('layouts/sidebar');
        $this->load->view('layouts/topbar');
        $this->load->view('dashboard/teknikelektro/edit', $data);
        $this->load->view('layouts/footer');
    }
    ...
    public function update($id)
    {
        ...
        if ($this->form_validation->run() === false) {
            ...
            if (!empty($_FILES['foto']['name'])) {
                ...
                if (!$this->upload->do_upload('foto')) {
                    $this->session->set_flashdata('error', $this->upload->display_errors());
                    redirect('teknikelektro/edit/' . $id);
                } else {
                    $fileData = $this->upload->data();
                    $foto = $fileData['file_name'];
                }
        ...
        'tendik_kontrak' => htmlspecialchars($this->input->post('tendik_kontrak')),
            ...
            $this->teknikelektro->updateData($id, $data);
    ...
    {
        $this->teknikelektro->destroyData($id);

        if ($this->db->affected_rows() > 0) {
            $this->session->set_flashdata('success', 'Berhasil menghapus teknikelektro!');
            redirect('teknikelektro');
        } else {
            $this->session->set_flashdata('error', 'Gagal menghapus teknikelektro!');
            redirect('teknikelektro');
        }
    }
    ...
    {
        $data['title'] = 'Teknik elektro | Detail Teknik elektro';
        $data['data'] = $this->teknikelektro->showData($id);

        $this->load->view('layouts/header', $data);
        $this->load->view('layouts/sidebar');
        $this->load->view('layouts/topbar');
        $this->load->view('dashboard/teknikelektro/show', $data);
        $this->load->view('layouts/footer');
    }
